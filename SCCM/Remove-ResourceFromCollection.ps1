    ...
    #region Validate Collection (first by ID then by Name)
        try{
            ...
            if($CollObj.Count -eq 1){
                $CollObj = $CollObj[0]
                if($CollObj.CollectionType -eq 1){ Write-Host "Found user collection $($CollObj.Name)" }
                else{ Write-Host "Found device collection $($CollObj.Name)" }
            }
        }
    ...
    #region Validate SCCM resource
        if($CollObj.Name -ne $null){
            try{
                ...
                if($ResObj.Count -eq 1 -and $CollObj.CollectionType -eq 2){ $ResObj = $ResObj[0]; Write-Host "Found computer $($ResObj.Name) with resource id $($ResObj.ResourceId)" }
                ...
                if($ResObj.Count -eq 1){ $ResObj = $ResObj[0];Write-Host "Found user $($ResObj.Name) with resource id $($ResObj.ResourceId)" }
        ...
        if($CollObj.Name -ne $null -and $ResObj.Name -ne $null){
            if($CollObj.CollectionType -eq 2 -and $ResObj.ResourceType -eq 5){
                ...
                try{
                    ...
                    Write-Host "Removed computer $($ResObj.Name) from collection $($CollObj.Name)"
                }
            ...
            if($CollObj.CollectionType -eq 1 -and $ResObj.ResourceType -eq 4){
                ...
                try{
                    ...
                    $CollObj.DeleteMemberShipRule($CollRule) | Out-Null
                    if($IncludeUpdateCollectionMembership){ $CollObj.RequestRefresh() | Out-Null }
                    Write-Host "Removed user $($ResObj.Name) from collection $($CollObj.Name)"
                }
                catch{ Write-Error "Error occurred when removing $Resource from collection $Collection. Exception: $($_.Exception.Message)" }
            }
        }
