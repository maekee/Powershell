...
#>
    ...
    #region Validate Device Collection (first by ID then by Name)
        try{
            ...
            if($CollObj.Count -eq 1){ $CollObj = $CollObj[0];Write-Verbose "Found device collection $($CollObj.Name)"}
        }
    ...
    #region Validate SCCM resource
        try{
            ...
            if($ResObj.Count -eq 1){ $ResObj = $ResObj[0]; Write-Verbose "Found computer $($ResObj.Name) with resource id $($ResObj.ResourceId)" }
        }
    ...
    #region Add Device (computer) to collection
        if($ResObj -ne $null){
            try{
                ...
                Write-Verbose "Added computer $($ResObj.Name) to collection $($CollObj.Name)"
            }
